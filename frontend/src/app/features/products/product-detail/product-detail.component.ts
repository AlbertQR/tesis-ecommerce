...
} from '@shared/components/product-reviews/product-reviews.component';
      ...
      <div class="container mx-auto px-4">
        ...
        <nav class="mb-6">
          <ol class="flex items-center gap-2 text-sm">
            ...
            <li>
              <a routerLink="/productos" class="text-brand hover:underline">Productos</a>
            </li>
            <li class="text-gray-400">/</li>
            ...
            <div class="grid md:grid-cols-2 gap-8 p-6">
              ...
              <div class="relative">
                ...
                <button (click)="toggleFavorite($event)"
                        class="absolute top-4 right-4 w-10 h-10 rounded-full bg-white
                        shadow-md hover:bg-brand hover:text-white flex items-center
                ...
                @if (product.isHot) {
                  <span
                    class="absolute top-4 left-4 bg-brand text-white text-sm font-bold
                     px-3 py-1 rounded">Hot</span>
                }
                @if (product.isCombo) {
                  <span
                    class="absolute top-4 left-4 bg-purple-600 text-white text-sm
                    font-bold px-3 py-1 rounded">Combo</span>
                }
                @if (product.category === 'despensa') {
                  <span
                    class="absolute top-4 left-4 bg-green-600 text-white text-sm
                    font-bold px-3 py-1 rounded">Despensa</span>
                }
              </div>
              ...
              <div>
                <span
                  class="text-sm text-gray-500 uppercase font-semibold">
                  {{ getCategoryLabel(product.category) }}
                </span>
                <h1 class="text-3xl font-bold text-gray-800 mt-1 mb-4">
                  {{ product.name }}
                </h1>
                ...
                <div class="flex items-center gap-4 mb-6">
                  <span
                    class="text-3xl font-bold text-brand">
                ...
                @if (!isAuthenticated()) {
                  <p class="text-sm text-gray-500 mt-4">
                    <a routerLink="/login" class="text-brand hover:underline">
                      Inicia sesión
...
export class ProductDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (id) {
      ...
      else this.router.navigate(['/productos']).then(() => {
      });
    }
  }
